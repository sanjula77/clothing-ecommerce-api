    ...
    try {
      ...
      if (!req.user) {
        ...
        if (process.env.NODE_ENV === 'test') {
          console.error(`[AUTH DEBUG] User not found for ID: ${decoded.id}, Type: ${typeof decoded.id}`);
          // Try to find any users in the database
          const allUsers = await User.find({});
          console.error(`[AUTH DEBUG] Total users in DB: ${allUsers.length}`);
          if (allUsers.length > 0) {
            console.error(`[AUTH DEBUG] Sample user IDs: ${allUsers.slice(0, 3).map(u => u._id.toString()).join(', ')}`);
          }
        }
