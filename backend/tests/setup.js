  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isDnsError && attempt < maxRetries) {
        ...
        console.warn(
          `⚠️  DNS resolution failed (attempt ${attempt}/${maxRetries}). Retrying in ${waitTime}ms...`
        );
      ...
      if (!isDnsError || attempt === maxRetries) {
        ...
        console.error(
          "   1. Check MongoDB Atlas Network Access - allow your IP or 0.0.0.0/0"
        );
        ...
        console.error(
          "   4. MongoDB Atlas will auto-create the database on first write"
        );
        console.error(
          "   5. DNS resolution issues may be network-related - check internet connection"
        );
        throw error;
      }
    }
